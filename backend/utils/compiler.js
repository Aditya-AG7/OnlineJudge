const fs = require('fs');
const path = require('path');
const crypto = require('crypto');
const { exec, execFile, spawn } = require('child_process');
const languages = require('../config/languages');

/**
 * Compiles a C++ source file to an executable output binary (legacy helper).
 */
function compileCpp(cppFile, outFile) {
  return new Promise((resolve, reject) => {
    execFile('g++', [cppFile, '-o', outFile], (err, stdout, stderr) => {
      if (err) {
        if (err.code === 'ENOENT') {
          return reject(new Error('g++ compiler is not installed or not found in PATH'));
        }
        return resolve({ success: false, compileError: stderr || err.message || 'Compilation error' });
      }
      return resolve({ success: true });
    });
  });
}

/**
 * Runs a compiled binary executable with stdin input and execution timeout (legacy helper).
 */
function runBinary(outFile, inputData = '', timeoutMs = 5000, cwd = process.cwd()) {
  return new Promise((resolve, reject) => {
    const child = spawn(outFile, [], { cwd });

    let stdout = '';
    let stderr = '';
    let isTimedOut = false;

    const timer = setTimeout(() => {
      isTimedOut = true;
      child.kill('SIGKILL');
    }, timeoutMs);

    child.stdout.on('data', (data) => {
      stdout += data.toString();
    });

    child.stderr.on('data', (data) => {
      stderr += data.toString();
    });

    child.on('error', (err) => {
      clearTimeout(timer);
      reject(err);
    });

    child.on('close', (code) => {
      clearTimeout(timer);
      if (isTimedOut) {
        return resolve({ status: 'TimeLimitExceeded', output: stdout, stderr: stderr, code: null });
      }
      if (code === 0) {
        return resolve({ status: 'Success', output: stdout, stderr: stderr, code: 0 });
      } else {
        return resolve({ status: 'RuntimeError', output: stdout, stderr: stderr, code: code });
      }
    });

    if (inputData) {
      child.stdin.write(inputData);
    }
    child.stdin.end();
  });
}

/**
 * Helper to execute a string compile command.
 */
function compileCommand(cmd, cwd) {
  return new Promise((resolve) => {
    exec(cmd, { cwd }, (err, stdout, stderr) => {
      if (err) {
        return resolve({
          success: false,
          compileError: stderr || err.message || 'Compilation error',
        });
      }
      return resolve({ success: true });
    });
  });
}

/**
 * Helper to run a command line process with timeout, stdin, and parse-error classification.
 */
function runCommand(runCmd, inputData = '', timeoutMs = 5000, cwd = process.cwd(), isParseErrorFn = null) {
  return new Promise((resolve, reject) => {
    const parts = runCmd.split(' ');
    const cmd = parts[0];
    const args = parts.slice(1);

    const child = spawn(cmd, args, { cwd });

    let stdout = '';
    let stderr = '';
    let isTimedOut = false;

    const timer = setTimeout(() => {
      isTimedOut = true;
      child.kill('SIGKILL');
    }, timeoutMs);

    child.stdout.on('data', (data) => {
      stdout += data.toString();
    });

    child.stderr.on('data', (data) => {
      stderr += data.toString();
    });

    child.on('error', (err) => {
      clearTimeout(timer);
      reject(err);
    });

    child.on('close', (code) => {
      clearTimeout(timer);
      if (isTimedOut) {
        return resolve({ status: 'TimeLimitExceeded', output: stdout, stderr: stderr, code: null });
      }
      if (code === 0) {
        return resolve({ status: 'Success', output: stdout, stderr: stderr, code: 0 });
      } else {
        if (isParseErrorFn && isParseErrorFn(stderr)) {
          return resolve({ status: 'CompileError', compileError: stderr, output: stdout, stderr: stderr, code: code });
        }
        return resolve({ status: 'RuntimeError', output: stdout, stderr: stderr, code: code });
      }
    });

    if (inputData) {
      child.stdin.write(inputData);
    }
    child.stdin.end();
  });
}

module.exports = {
  compileCpp,
  runBinary,
  compileCommand,
  runCommand,
};
